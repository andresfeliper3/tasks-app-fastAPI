...
@task_router.get('/', response_model=List[Task], status_code=200)#, dependencies=[Depends(JWTBearer())])
...
def get_tasks_by_category(category: str = Query(min_length=3), db = Depends(get_db)) -> List[Task]:
