...
def get_category_by_id(category_id: int = Path(..., ge=1), db = Depends(get_db)) -> Category:
...
def update_category(category_id: int, category: Category, db = Depends(get_db)) -> Category:
